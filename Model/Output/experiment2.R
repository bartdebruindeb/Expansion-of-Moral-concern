...
filesNames <- ("Model2.0 Experiment 2-table.csv")
...
clean_df <- clean_df %>% group_by(tick, Intersubjective_scenario) %>% 
  summarise(across( Stage_1.00:Max_5_Friends  , mean))
...
df_long <- gather(clean_df, variable, measurement, Stage_1.00:Max_5_Friends)
...
# Value_change --------------------------------------------------
 ...
 # scale_color_manual(name = "Stages of Moral Expansion",
  ...
  facet_grid(. ~ "Low Group Pressure (0.5)") +
  #guides(color = guide_legend(override.aes = list(size=9, alpha=1, fill=NA))) +
 
  theme_ipsum() +
  theme(strip.background = element_rect(fill="black"),
        strip.text = element_text(size=12, colour="white", face = "bold", hjust = 0.5))
...
MEHOL
 ...
 MEHOE <- df_long%>% filter((
   ...
   # scale_color_manual(name = "Stages of Moral Expansion",
   #                   values = colorRampPalette(brewer.pal(11, "Spectral"))(9)) +
   facet_grid(. ~ "Medium Group Pressure (1.5)") +
   theme_ipsum() +
   theme(strip.background = element_rect(fill="black"),
         strip.text = element_text(size=12, colour="white", face = "bold", hjust = 0.5))
 ...
 MEHEL <- df_long%>% filter((
   ...
   # scale_color_manual(name = "Stages of Moral Expansion",
   #                   values = colorRampPalette(brewer.pal(11, "Spectral"))(9)) +
   facet_grid(. ~ "High Group Pressure (2.5)") +
   theme_ipsum() +
   theme(strip.background = element_rect(fill="black"),
         strip.text = element_text(size=12, colour="white", face = "bold", hjust = 0.5))
 MEHEL
 ...
 LM <- df_long%>% filter((
   variable == "Min_5_Friends" |
     variable == "Max_5_Friends" ),
   Intersubjective_scenario == 0.5) %>%
   dplyr::sample_frac(1) %>%
   ggplot( aes(x = tick,
               y = measurement, fill = variable, color = variable)) +
   geom_line(linewidth=0.75,alpha=1) +
   #geom_area(position = "fill", colour = "black", size = .2, alpha = .4) +
   # stat_smooth(
   #    geom = 'area', span = 0.2,
   #   alpha = 0.4) + 
   xlab("t (Weeks)") +
   ylab("Moral Expansion") + 
#   scale_fill_viridis(discrete = T) +
   scale_x_continuous() +
   scale_y_continuous() +
   scale_fill_manual(name = "Stages of Moral Expansion",
...
DM <-  ggarrange (AM,BM, CM,
                  labels = c("A", "B", "C"),
                  ncol = 1, nrow = 3,
                  legend = "right",
                  common.legend = TRUE)
DM
IM <- ggarrange (LM,MM, HM,
                labels = c("A", "B", "C"),
                ncol = 1, nrow = 3,
                legend = "right",
                common.legend = TRUE)
IM
 ...
 esfigure <- ggarrange (MEHOL, MEHOE, MEHEL,
                        labels = c("A", "B", "C"),
                        ncol = 1, nrow = 3,
                        legend = "right",
                        common.legend = TRUE)
