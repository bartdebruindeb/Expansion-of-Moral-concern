...
filesNames <- ("Model2.0 Experiment 3-table.csv")
...
clean_df <- clean_df %>% group_by( tick, Tipping_Strategy) %>% 
...
N2 <- df_long%>% filter((
   ...
   facet_grid(. ~ "No Tipping Strategy") +
...
print(G2_T3)
 ...
 G2  <- df_long%>% filter((
   ...
   # scale_color_manual(name = "Stages of Moral Expansion",
   #                   values = colorRampPalette(brewer.pal(11, "Spectral"))(9)) +
   facet_grid(. ~ "Tipping Strategy = Shotgun") +
 ...
 T2 <- df_long%>% filter((
   variable == "Stage_1.00" |
     ...
     variable == "Stage_2.75" |
     variable == "Stage_3.00" ),
   Tipping_Strategy == "Silver Bullets" ) %>%
   dplyr::sample_frac(1) %>%
   ggplot( aes(x = tick,
               y = measurement, fill = variable)) +
   # geom_line(linewidth=0.75,alpha=1) +
   ...
   # scale_color_manual(name = "Stages of Moral Expansion",
   #                   values = colorRampPalette(brewer.pal(11, "Spectral"))(9)) +
   facet_grid(. ~ "Tipping Strategy = Silver Bullets") +
 ...
 L2 <- df_long%>% filter((
   ...
   # scale_color_manual(name = "Stages of Moral Expansion",
   #                   values = colorRampPalette(brewer.pal(11, "Spectral"))(9)) +
   facet_grid(. ~ "Tipping Strategy = Snowball") +
