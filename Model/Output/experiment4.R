...
filesNames <- ("Model2.0 Experiment 4-table.csv")
...
N2_T1 <- df_long%>% filter((
  variable == "Stage_1.00"),  Tipping_Strategy == 0 ) 
N2_T1 <- which.max(N2_T1$measurement <= 1000)
N2_T2 <- df_long%>% filter((
  variable == "Stage_2.00"), Tipping_Strategy == 0)
N2_T2 <- which.max(N2_T2$measurement > 0)
N2_T3 <- df_long%>% filter((
  variable == "Stage_3.00"), Tipping_Strategy == 0)
...
G2_T1 <- df_long%>% filter((
  variable == "Stage_1.00"),  Tipping_Strategy == 0.02 ) 
G2_T1 <- which.max(G2_T1$measurement <= 1000)
G2_T2 <- df_long%>% filter((
  variable == "Stage_2.00"), Tipping_Strategy == 0.02)
G2_T2 <- which.max(G2_T2$measurement >= 41)
G2_T3 <- df_long%>% filter((
  variable == "Stage_3.00"), Tipping_Strategy == 0.02)
...
print(G2_T3)
 ...
 L2_T1 <- df_long%>% filter((
   variable == "Stage_1.00"),  Tipping_Strategy == 0.1 ) 
 L2_T1 <- which.max(L2_T1$measurement <= 1000)
 L2_T2 <- df_long%>% filter((
   variable == "Stage_2.00"), Tipping_Strategy == 0.1)
 L2_T2 <- which.max(L2_T2$measurement >= 40)
 L2_T3 <- df_long%>% filter((
   variable == "Stage_3.00"), Tipping_Strategy == 0.1)
 ...
 L2 <- df_long%>% filter((
   ...
   # scale_color_manual(name = "Stages of Moral Expansion",
   #                   values = colorRampPalette(brewer.pal(11, "Spectral"))(9)) +
   facet_grid(. ~ "Agents-of-Change = 10%") +
  ...
  esfigure <- ggarrange (N2, G2,T2, L2,  
                        labels = c("A", "B", "C", "D"),
                        ncol = 2, nrow = 2,
                        legend = "right",
                        common.legend = TRUE)
