...
clean_df <- clean_df %>% group_by( tick, Tipping_Strategy, Group_Pressure) %>% 
  summarise(across( Stage_1.00:Min_5_Friends  , mean)) 
...
df_long <- gather(clean_df, variable, measurement, Stage_1.00:Min_5_Friends)
...
N2_T1 <- df_long%>% filter((
  variable == "Stage_1.00"),  Tipping_Strategy == "none" ) 
N2_T1 <- which.max(N2_T1$measurement <= 1000)
N2_T2 <- df_long%>% filter((
  variable == "Stage_2.00"), Tipping_Strategy == "none")
N2_T2 <- which.max(N2_T2$measurement > 0)
N2_T3 <- df_long%>% filter((
  variable == "Stage_3.00"), Tipping_Strategy == "none")
N2_T3 <- which.max(N2_T3$measurement >= 1)
print(N2_T3)

N2_T1 <- as.numeric(N2_T1)
N2_T2 <- as.numeric(N2_T2)
N2_T3 <- as.numeric(N2_T3)
...
N2 <- df_long%>% filter((
   variable == "Stage_1.00" |
     ...
     variable == "Stage_2.75" |
     variable == "Stage_3.00" ),
   Tipping_Strategy == "none", Group_Pressure == 2.5) %>%
   dplyr::sample_frac(1) %>%
   ggplot( aes(x = tick,
   ...
   facet_grid(. ~ "No TS, GP = 2.5") +
   #guides(color = guide_legend(override.aes = list(size=9, alpha=1, fill=NA))) +
    geom_vline(xintercept=N2_T1, colour="black", size = 1.5) +
   geom_vline(xintercept=N2_T2, colour="black", size = 1.5) +
  geom_vline(xintercept=N2_T3, colour="black", size = 1.5) +
 # geom_text(aes(x=N2_T, label="T1\n"), angle = 90, colour="black", size = 2)+
  annotate("text", x = N2_T1, y = 0.7:1.0, angle = 90, label = paste("T1 =", N2_T1), vjust = -0.8, size = 6, fontface =2)+
  annotate("text", x = N2_T2, y = 0.7:1.0, angle = 90, label = paste("T2 =", N2_T2), vjust = -0.8, size = 6, fontface =2)+
  annotate("text", x = N2_T3, y = 0.7:1.0, angle = 90, label = paste("T3 =", N2_T3), vjust = -0.8, size = 6, fontface =2)+
...
G2_T1 <- df_long%>% filter((
  variable == "Stage_1.00"),  Tipping_Strategy == "Shotgun" ) 
G2_T1 <- which.max(G2_T1$measurement <= 1000)
G2_T2 <- df_long%>% filter((
  variable == "Stage_2.00"), Tipping_Strategy == "Shotgun")
G2_T2 <- which.max(G2_T2$measurement >= 41)
G2_T3 <- df_long%>% filter((
  variable == "Stage_3.00"), Tipping_Strategy == "Shotgun")
G2_T3 <- which.max(G2_T3$measurement >= 1)
print(G2_T3)
 ...
 G2  <- df_long%>% filter((
   variable == "Stage_1.00" |
     ...
     variable == "Stage_2.75" |
     variable == "Stage_3.00" ),
   Tipping_Strategy == "Shotgun", Group_Pressure == 2.5 ) %>%
   dplyr::sample_frac(1) %>%
   ggplot( aes(x = tick,
               y = measurement, fill = variable)) +
   # geom_line(linewidth=0.75,alpha=1) +
   ...
   # scale_color_manual(name = "Stages of Moral Expansion",
   #                   values = colorRampPalette(brewer.pal(11, "Spectral"))(9)) +
   facet_grid(. ~ "TS = Shotgun, GP = 2.5") +
   
   geom_vline(xintercept=G2_T1, colour="black", size = 1.5) +
   geom_vline(xintercept=G2_T2, colour="black", size = 1.5) +
   geom_vline(xintercept=G2_T3, colour="black", size = 1.5) +
   # geom_text(aes(x=G2_T, label="T1\n"), angle = 90, colour="black", size = 2)+
   annotate("text", x = G2_T1, y = 0.7:1.0, angle = 90, label = paste("T1 =", G2_T1), vjust = -0.8, size = 6, fontface =2)+
   annotate("text", x = G2_T2, y = 0.7:1.0, angle = 90, label = paste("T2 =", G2_T2), vjust = 1.5, size = 6, fontface =2)+
   annotate("text", x = G2_T3, y = 0.7:1.0, angle = 90, label = paste("T3 =", G2_T3), vjust = -0.8, size = 6, fontface =2)+
 ...
 T2_T1 <- df_long%>% filter((
   variable == "Stage_1.00"),  Tipping_Strategy == "Silver Bullets" ) 
 T2_T1 <- which.max(T2_T1$measurement <= 1000)
 T2_T2 <- df_long%>% filter((
   variable == "Stage_2.00"), Tipping_Strategy == "Silver Bullets")
 T2_T2 <- which.max(as.numeric(T2_T2$measurement) >= 41)
 T2_T3 <- df_long%>% filter((
   variable == "Stage_3.00"), Tipping_Strategy == "Silver Bullets")
 T2_T3 <- which.max(T2_T3$measurement >= 1)
 print(T2_T3)
 ...
 T2 <- df_long%>% filter((
   variable == "Stage_1.00" |
     ...
     variable == "Stage_2.75" |
     variable == "Stage_3.00" ),
   Tipping_Strategy == "Silver Bullets", Group_Pressure == 2.5 ) %>%
   dplyr::sample_frac(1) %>%
   ggplot( aes(x = tick,
               y = measurement, fill = variable)) +
   # geom_line(linewidth=0.75,alpha=1) +
   ...
   # scale_color_manual(name = "Stages of Moral Expansion",
   #                   values = colorRampPalette(brewer.pal(11, "Spectral"))(9)) +
   facet_grid(. ~ "TS = Silver Bullets, GP = 2.5") +
   geom_vline(xintercept=T2_T1, colour="black", size = 1.5) +
   geom_vline(xintercept=T2_T2, colour="black", size = 1.5) +
   geom_vline(xintercept=T2_T3, colour="black", size = 1.5) +
   # geom_text(aes(x=T2_T, label="T1\n"), angle = 90, colour="black", size = 2)+
   annotate("text", x = T2_T1, y = 0.7:1.0, angle = 90, label = paste("T1 =", T2_T1), vjust = -0.8, size = 6, fontface =2)+
   annotate("text", x = T2_T2, y = 0.7:1.0, angle = 90, label = paste("T2 =", T2_T2), vjust = 1.5, size = 6, fontface =2)+
   annotate("text", x = T2_T3, y = 0.7:1.0, angle = 90, label = paste("T3 =", T2_T3), vjust = -0.8, size = 6, fontface =2)+
 ...
 L2_T1 <- df_long%>% filter((
   variable == "Stage_1.00"),  Tipping_Strategy == "Snowball" ) 
 L2_T1 <- which.max(L2_T1$measurement <= 1000)
 L2_T2 <- df_long%>% filter((
   variable == "Stage_2.00"), Tipping_Strategy == "Snowball")
 L2_T2 <- which.max(L2_T2$measurement >= 40)
 L2_T3 <- df_long%>% filter((
   variable == "Stage_3.00"), Tipping_Strategy == "Snowball")
 L2_T3 <- which.max(L2_T3$measurement >= 1)
 print(L2_T3)
 ...
 L2 <- df_long%>% filter((
   variable == "Stage_1.00" |
     ...
     variable == "Stage_2.75" |
     variable == "Stage_3.00" ),
   Tipping_Strategy == "Snowball", Group_Pressure == 2.5 ) %>%
   dplyr::sample_frac(1) %>%
   ggplot( aes(x = tick,
               y = measurement, fill = variable)) +
   # geom_line(linewidth=0.75,alpha=1) +
   ...
   # scale_color_manual(name = "Stages of Moral Expansion",
   #                   values = colorRampPalette(brewer.pal(11, "Spectral"))(9)) +
   facet_grid(. ~ "TS = Snowball, GP = 2.5") +
   geom_vline(xintercept=L2_T1, colour="black", size = 1.5) +
   geom_vline(xintercept=L2_T2, colour="black", size = 1.5) +
   geom_vline(xintercept=L2_T3, colour="black", size = 1.5) +
   # geom_text(aes(x=L2_T, label="T1\n"), angle = 90, colour="black", size = 2)+
   annotate("text", x = L2_T1, y = 0.7:1.0, angle = 90, label = paste("T1 =", L2_T1), vjust = -0.8, size = 6, fontface =2)+
   annotate("text", x = L2_T2, y = 0.7:1.0, angle = 90, label = paste("T2 =", L2_T2), vjust = -0.8, size = 6, fontface =2)+
   annotate("text", x = L2_T3, y = 0.7:1.0, angle = 90, label = paste("T3 =", L2_T3), vjust = 1.5, size = 6, fontface =2)+
  ...
  esfigure <- ggarrange (N2, T2, L2, G2,
                        labels = c("A", "B", "C", "D"),
                        ncol = 2, nrow = 2,
                        legend = "right",
                        common.legend = TRUE)
